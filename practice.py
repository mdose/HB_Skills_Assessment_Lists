...
def smallest_int(numbers):
    ...
    **DO NOT USE** the built-in function `min()`!
    ...
    #UGH. This one and the one below were evil. Figuring out how to compare 
    #numbers in without going out of index range was a nightmare.

    index = 0
    if numbers == []:
        smallest_int = None
    else:
        smallest_int = int(numbers[0])

        while index < len(numbers):
            try:
                comparing_number = int(numbers[index + 1])

            except IndexError:
                break

            if smallest_int <= comparing_number:
                smallest_int = smallest_int
            elif smallest_int >= comparing_number:
                smallest_int = comparing_number
            else:
                break
    ...
    **DO NOT USE** the built-in function `max()`!
    ...
    if numbers == []:
        ...
        while index < len(numbers):
            ...
            if largest_int >= comparing_number:
                largest_int = largest_int
            elif largest_int <= comparing_number:
                largest_int = comparing_number
            else:
                break
